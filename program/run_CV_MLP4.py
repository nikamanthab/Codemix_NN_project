...
from train_4L import train
from neural_net_4L import LinearNetwork
import wandb
import torch
...
mlp_dropout=[0,0.25]
mlp_hiddenNodes = [(1024,512,256),(512,256,128)]
            ...
            for nmf in ngram_maxfeatures:
                for dp in mlp_dropout:
                    for hn in mlp_hiddenNodes:
                        fname = tag[0]+"_"+tag[2]+"_"+na+"_"+str(nr[0])+"-"+str(nr[1])+"_"+str(nmf)+"_4L_"+str(dp)+"D_"+str(hn[0])+"_"+str(hn[1])+"_"+str(hn[2])
                        print(fname)
                        run = wandb.init(project="CountVectorizer_MLP4", entity="nnproj",reinit=True,name=fname)
                        ...
                        train(LinearNetwork, vectorizer, 'CountVectorizer', train_df, val_df, epochs=EPOCHS, \
                              learning_rate=ETA, batch_size=BATCH_SIZE, log=wandb.log, device=tdevice,hidden1=hn[0],hidden2=hn[1],hidden3=hn[2],dropout=dp)
